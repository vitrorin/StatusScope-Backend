...
class GetDiagnosisAssistantThreadUseCaseTest {
    ...
    void shouldReturnThreadMessagesAndContextForDoctor() {
        ...
        hospital.setRegionId(REGION_ID);

        Mockito.when(hospitalRepository.findHospitalById(HOSPITAL_ID)).thenReturn(Optional.of(hospital));
        Mockito.when(outbreakRepository.findActiveByRegionId(REGION_ID)).thenReturn(List.of(outbreak));
        ...
        Assertions.assertEquals("Region Norte", dto.getContextUsed().getRegionName());
